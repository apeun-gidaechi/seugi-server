...
class OAuthAdapter (
    ...
    override fun loadOAuth(id: Long, provider: String): OAuth {
        val member = memberRepository.findByIdOrNull(id)
            ?: throw CustomException(MemberErrorCode.MEMBER_NOT_FOUND)

        val oAuth = oAuthRepository.findByMemberIdAndProvider(member, provider)
            ?: throw CustomException(OAuthErrorCode.OAUTH_NOT_FOUND)

        return oAuthMapper.toDomain(oAuth)
    }

    override fun existOAuth(id: Long, provider: String): Boolean {
        val member = memberRepository.findByIdOrNull(id)
            ?: throw  CustomException(MemberErrorCode.MEMBER_NOT_FOUND)

        return oAuthRepository.existsByMemberIdAndProvider(member, provider)
    }
